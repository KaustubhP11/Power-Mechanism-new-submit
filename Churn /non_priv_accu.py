...
def main():
    ...
    m1 = torch.load("Models/net_5_2.pth")
    ...
    eps_list = [2,2.5,3,3.5,4.5,5.5,6]
    acc_list =[]
    for eps in eps_list:
        set_eps = eps
        ...
        y_pred = model(X[ind:-1].cuda())
        accuracy3 = (y_pred.round() == Y[ind:-1].cuda()).float().mean()
        # print(f"Test Accuracy 2 {accuracy}")
        acc_list.append([str(eps),str(accuracy1),str(accuracy2),str(accuracy3)])
        # for row in acc_list:
        #     print('\t'.join(row))
            # try:
            #     print('\t'.join(row))
